...
@section('page-title', __('store.edit'))
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 max-sm:p-4">
        <form action="{{ route('stores.update', $store) }}" method="POST" class="space-y-6 max-sm:space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    >
                        @foreach($businessTypes as $key => $config)
                            <option value="{{ $key }}" {{ old('business_type', $store->business_type) === $key ? 'selected' : '' }}>
                                {{ $config['name'] ?? $key }}
                            </option>
                        @endforeach
                    </select>
                ...
                @if(!auth()->user()->isAdmin())
                    <input type="hidden" name="business_type" value="{{ $store->business_type }}">
                @endif
                @error('business_type')
                    <p class="mt-1 text-sm max-sm:text-xs text-red-500">{{ $message }}</p>
                @enderror
            ...
            @if(auth()->user()->isAdmin())
                    ...
                    <div class="relative">
                        ...
                        >
                            ...
                            @foreach($owners as $owner)
                                <option value="{{ $owner->id }}" {{ (string) old('owner_id', $store->owner_id) === (string) $owner->id ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-2 max-sm:grid-cols-1 gap-4">
                    <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $store->is_active) ? 'checked' : '' }} class="w-4 h-4 {{ $tc->checkbox ?? 'text-rose-500' }} border-gray-300 dark:border-gray-600 rounded">
                        {{ __('store.active') }}
                    </label>
                    <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                        <input type="hidden" name="is_approved" value="0">
                        <input type="checkbox" name="is_approved" value="1" {{ old('is_approved', $store->is_approved) ? 'checked' : '' }} class="w-4 h-4 {{ $tc->checkbox ?? 'text-rose-500' }} border-gray-300 dark:border-gray-600 rounded">
                        {{ __('store.approved') }}
                    </label>
                </div>
            @endif
